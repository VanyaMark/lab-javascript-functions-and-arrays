...
function sum(someArray) {
  ...
  for (let i = 0; i< someArray.length; i++) {
    ...
    if (typeof arrayElement == 'number') {
      ...
      sumElements += arrayElement.length;
    }
...
function averageNumbers(someArray) {
  ...
  let arrayAverage = sum / someArray.length;
...
function averageWordLength(words) { 
  ...
  averageWordLength = wordLengthSum / words.length;
...
function avg(arr) {
  ...
  let averageArr = sumElements / arr.length;
  return averageArr;
}
...
function doesWordExist(wordsArray,searchedWord) {
  if (wordsArray.length == 0) {
   ...
   if (wordsArray.includes(searchedWord) == true) {
    return true;
   }
   else {
    return false;
...
function greatestProduct(matrix) {
  let greatestProductOfFour = 0;
  let multiplyContainer1 = 0;
  let multiplyContainer2 = 0;

  for (let i = 0; i < matrix.length; i++) {
    for (let j = 0; j < matrix.length - 3; j++) {
      multiplyContainer1 =
        matrix[i][j] * matrix[i][j + 1] * matrix[i][j + 2] * matrix[i][j + 3];
      if (multiplyContainer1 >= greatestProductOfFour) {
        greatestProductOfFour = multiplyContainer1;
      }
    }
  }
  for (j = 0; j < matrix.length; j++) {
    for (i = 0; i < matrix.length - 3; i++) {
      ...
      if (multiplyContainer2 >= greatestProductOfFour) {
        greatestProductOfFour = multiplyContainer2;
      }
    }
  }

  return greatestProductOfFour;
}
